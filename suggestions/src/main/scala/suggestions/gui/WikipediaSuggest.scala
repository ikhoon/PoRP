...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered(t => ObservableEx(wikipediaSuggestion(t)))
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.sanitized.concatRecovered(selection => ObservableEx(wikipediaPage(selection)))
